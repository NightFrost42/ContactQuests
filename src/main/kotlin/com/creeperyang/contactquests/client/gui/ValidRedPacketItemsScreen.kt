...
class ValidRedPacketItemsScreen(): BaseScreen() {
    private var title: Component? = null
    private lateinit var itemPanel: Panel
    private lateinit var backButton: Button
    private lateinit var submitButton: Button
    private lateinit var task: RedPacketTask

    constructor (task: RedPacketTask, validItems: MutableList<ItemStack>) : this() {
        this.task = task
        title = Component.translatable("contactquest.task.ftbquests.redpacket.valid_for", task.title)

        itemPanel = object : Panel(this) {
            override fun addWidgets() {
                for (validItem in validItems) {
                    add(ValidRedPacketButton(this, validItem))
                }
            }
        ...
        submitButton = object : SimpleTextButton(this, Component.translatable("contactquest.gui.submit"), Color4I.empty()) {
            ...
            override fun onClicked(button: MouseButton?) {
                ...
                val slot = player.inventory.items.indexOfFirst {
                    !it.isEmpty && (it.item is RedPacketEnvelopeItem)
                }

                if (slot == -1) {
                    ContactQuests.debug("ValidRedPacketItemsScreen: 未找到红包纸")
                    return
                }
                ...
                ContactQuests.debug("ValidRedPacketItemsScreen: 准备打开槽位: $slot")

                RedPacketAutoFiller.schedule(task)

                executeOpenRedPacketInteraction(mc, player, connection, slot)
            }

            private fun executeOpenRedPacketInteraction(
                mc: Minecraft,
                player: Player,
    ...
    private class ValidRedPacketButton : Button {
